...
import { PropertiesPanel, DebounceInputModule, usePrevious, SelectEntry, isCheckboxEntryEdited, isSelectEntryEdited, CheckboxEntry, isTextAreaEntryEdited, TextAreaEntry, isTextFieldEntryEdited, TextFieldEntry, Group, FeelEntry, FeelTextAreaEntry, isFeelEntryEdited, isToggleSwitchEntryEdited, ToggleSwitchEntry, ListGroup, ListEntry, SimpleEntry, CollapsibleEntry, useLayoutState, HeaderButton, ArrowIcon, CreateIcon, DropdownButton } from '@bpmn-io/properties-panel';
...
const FeelEntryWithContext = withVariableContext(withTooltipContainer(FeelEntry));
const FeelTextAreaEntryWithContext = withVariableContext(withTooltipContainer(FeelTextAreaEntry));
...
function isZeebeServiceTask(element) {
  ...
  if (isAny$1(element, ['bpmn:BusinessRuleTask', 'bpmn:ScriptTask']) && !getTaskDefinition$2(element)) {
    return false;
  }
  return true;
}
...
function ValueProperty$1(props) {
  ...
  return TextFieldEntry({
    element: property,
    id: idPrefix + '-value',
    label: translate('Type'), //MANUALLY MODIFIED
    getValue,
    setValue,
    debounce
  });
